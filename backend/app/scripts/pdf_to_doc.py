    ...
    try:
        ...
        for i, para in enumerate(doc.paragraphs):
            ...
            if not text:
                ...
                # 3. 规范化行距（避免过大行距导致的混乱）
                if (
                    para.paragraph_format.line_spacing is None
                    or para.paragraph_format.line_spacing > 2.0
                ):
                ...
                if (
                    ...
                    and para.paragraph_format.first_line_indent.pt > 100
                ):
...
def main():
    parser = argparse.ArgumentParser(description="PDF 转 Word (增强版) - 保留图片、格式和样式")
