...
const { downloadFile: downloadFileUtil, chooseMessageFile } = require('../../utils/file');
const {
  normalizeFileUrl,
...
Page({
  data: {
    ...
    // 转换映射
    conversionMap: { ...IMAGE_CONVERSION_MAP },
    ...
    // 格式显示名称
    formatDisplayNames: IMAGE_FORMAT_DISPLAY_NAMES
  },
  ...
  async loadFormats() {
    try {
      const result = await loadSupportedFormats('image');
      if (result?.image?.supportedConversions) {
        const rawMap = result.image.supportedConversions;
        const normalizedMap = {};
  ...
  selectSourceFormat(e) {
    ...
    const itemDisplayNames = getTargetDisplayNames('image', availableTargets);
  ...
  async startConvert() {
    ...
    for (let i = 0; i < this.data.fileList.length; i++) {
      ...
      try {
        ...
        const task = await createImageConvertTask({
          filePath: item.path,
          targetFormat: target
        });
  ...
  async previewFile(e) {
    ...
    // 图片可以直接预览
    const ext = getExt(item.downloadUrl.split('?')[0]).toLowerCase();
